...
export const load = async ({ parent }) => {
  ...
  const { data: seasons, error: err } = await supabase.rpc("run_query", {
    query: `
    select seasons.*, row_to_json(schools) as school, count(*) from class_season_level_courses 
    inner join classes on classes.id = class_season_level_courses.class_id 
    inner join seasons on seasons.id = classes.season_id
    inner join schools on schools.id = seasons.school_id
    where class_season_level_courses.person_dni = '${session.user.user_metadata.dni}'
