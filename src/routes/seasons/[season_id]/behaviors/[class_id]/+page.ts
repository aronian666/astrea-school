...
export const load = async ({ params: { class_id }, parent }) => {
  ...
  const { data: clas, error: err } = await supabase.from("classes").select("level:levels(name), grade, section:sections(name), seasons(id, name), class_persons(id, behaviors(*), person:persons(full_name))")
    .eq("id", class_id)
