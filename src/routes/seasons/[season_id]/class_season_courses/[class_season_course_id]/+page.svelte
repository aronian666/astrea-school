<script lang="ts">
  ...
  export let data;
  let cycle_id = data.class_season_level_courses.class?.season?.cycles[1].id;
  $: notes = data.class_season_level_courses.notes.filter((note) => cycle_id === note.cycle_id);
</script>

<hgroup>
  <h1>{data.class_season_level_courses.season_level_course?.course?.name}</h1>
  <small>
    {data.class_season_level_courses.class?.level?.name}
    {data.class_season_level_courses.class?.grade}
    {data.class_season_level_courses.class?.section?.name}
  </small>
</hgroup>
<form>
  <div class="grid auto-fit gap1" style="--width: 5rem">
    {#each data.class_season_level_courses.class?.season?.cycles || [] as cycle}
      <label>
...
<section class="grid">
  <table class="bordered">
    <thead>
      <tr>
        <th>Estudiante</th>
        {#each data.class_season_level_courses.season_level_course?.competences || [] as competence, index}
          <th>
    ...
    <tbody>
      {#each data.class_season_level_courses.class?.class_persons || [] as class_person, index}
        <tr class="input">
          <td>
            <span class="w500">
              {index + 1}
          ...
          {#each data.class_season_level_courses.season_level_course?.competences || [] as competence}
                ...
                on:change={async (e) => {
                  ...
                  const { error, data: new_note } = await data.supabase
                    ...
                    .upsert({
                      ...
                      class_season_level_course_id: data.class_season_level_courses.id,
                  ...
                  data.class_season_level_courses.notes.push(new_note);
                  data = data;
                }}
                on:input={(e) => {
                  const input = e.currentTarget;
