...
export const load = async ({ parent, params: { season_id } }) => {
  ...
  const { data: class_season_courses, error: err } = await supabase.from("class_season_courses").select("*, season_course:season_courses(course:courses(name)), classes!inner(grade, section_id, section:sections(name), level:levels(name, id, icon),season_id)").eq("person_dni", session.user.user_metadata.dni).eq("classes.season_id", season_id)
  ...
  return { class_season_courses, levels, courses }
};